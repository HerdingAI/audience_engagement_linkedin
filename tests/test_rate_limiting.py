...
def test_rate_limiting_logic():
    ...
    # Validate results
    assert len(filtered_posts) == 6, (
        f"Expected 6 posts, got {len(filtered_posts)}"
    )
    assert profile_counts[1] == 3, (
        f"Profile 1 should have 3 posts, got {profile_counts[1]}"
    )
    assert profile_counts[2] == 2, (
        f"Profile 2 should have 2 posts, got {profile_counts[2]}"
    )
    ...
    assert len(filtered_comments) == 4, (
        f"Expected 4 comments, got {len(filtered_comments)}"
    )
    assert profile_counts[1] == 2, (
        f"Profile 1 should have 2 comments, got {profile_counts[1]}"
    )
    assert profile_counts[2] == 2, (
        f"Profile 2 should have 2 comments, got {profile_counts[2]}"
    )
